...
import { Eye, FileDown, Filter, Pencil, Plus, Trash2 } from 'lucide-react'
    ...
    try {
      ...
      switch (bulkAction) {
        case 'updateStatus':
          await Promise.all(
            selectedProducts.map((id) =>
              updateProduct(id, {
                status: bulkActionValue as 'active' | 'hidden'
              })
            )
          )
          toast({
            title: 'Products updated',
            description: `Status updated for ${selectedProducts.length} products.`
          })
          break
        case 'delete':
        ...
        case 'updatePrice':
          ...
          toast({
            title: 'Products updated',
            description: `Price updated for ${selectedProducts.length} products.`
          })
          break
        case 'updateStock':
          await Promise.all(
          ...
          toast({
            title: 'Products updated',
            description: `Stock updated for ${selectedProducts.length} products.`
          })
          break
      }
    } catch (error) {
    ...
    <div className='space-y-4'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2'>
        <div className='flex-1 w-full sm:w-auto'>
          <Input
            placeholder='Search by name, perfume code...'
          ...
          <CardContent>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {Array.from(new Set(products.map((p) => p.origin))).map(
                      (origin) => (
              ...
              <div>
                ...
                <Select
                  value={filters.isHot}
                  onValueChange={(value) => handleFilterChange('isHot', value)}
                >
            ...
            <div className='flex items-center space-x-2'>
              ...
              {(bulkAction === 'updatePrice' ||
                ...
                <Input
                  type='text'
                  placeholder={`Enter ${
                    bulkAction === 'updatePrice' ? 'price' : 'stock'
                  } change`}
                  value={bulkActionValue}
            ...
            <TableBody>
              {paginatedProducts.map((product) => (
                <TableRow key={product.id}>
                  ...
                  <TableCell>
                    {brands.find((b) => b.id === product.fragrance_brand_id)
                      ?.name || 'Unknown'}
                  </TableCell>
